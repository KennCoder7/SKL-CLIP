...
class REC_Processor(Processor):
    ...
    def train(self):
        ...
        for data, label, support_data, support_label in tqdm(loader):
            ...
            # forward
            cls, cls_e, emb_norm, emb_e_norm, y_txt, y_txt_froster, y_txt_stu, domain, domain_e = self.model(True, xs, xse, label)
            ...
            loss_scl = self.arg.w_scl * self.supcl(skl_scl_feat, label)
            loss_clip = self.arg.w_clip * (self.loss_clip(emb_norm, y_txt, label) +
                                           self.loss_clip(emb_e_norm, y_txt, label))
            if y_txt_froster is not None:
                loss_froster = self.arg.w_froster * self.loss_fd_distill(y_txt_froster.detach(), y_txt_stu)
            else:
            ...
            if self.arg.detach_support_text:
                loss_clip_Spt = self.arg.w_clip * (self.loss_clip(emb_norm_spt, y_txt_spt.detach(), support_label) +
                                                   self.loss_clip(emb_e_norm_spt, y_txt_spt.detach(), support_label))
            else:
            ...
            loss = self.arg.w_balance * (loss_cls + loss_scl + loss_clip + loss_froster) + self.arg.w_balance_s * (
    ...
    def test(self, evaluation=True):
        self.model.eval()
        loader = self.data_loader['zsl_test']
        loss_value = [0.]
        loss_txt_value = [0.]
        result_frag = []
        label_frag = []
        ...
        if self.arg.zsl_test_feeder_args:
            num = 0
            for data, label in tqdm(loader):
                ...
                # inference
                with torch.no_grad():
                    _, txt_sim = self.model(True, data, unseen=True)
        ...
        seen_correct = 0
        seen_num = 0
        if self.arg.gzsl_test_feeder_args:
            unseen_list = self.model.unseen_class
            seen_list = list(set(np.arange(self.model.target_num_class)) - set(unseen_list))
            num_s = 0
            loader_s = self.data_loader['gzsl_test']
            for data, label in tqdm(loader_s):
                data = data.float().to(self.dev)
                label = label.long().to(self.dev)
                with torch.no_grad():
                    _, txt_sim = self.model(True, data, unseen=True, gzsl=True)
                txt_sim = torch.nn.functional.softmax(txt_sim, dim=1)
                txt_sim_seen = torch.zeros_like(txt_sim)
                txt_sim_unseen = torch.zeros_like(txt_sim)
                txt_sim_seen[:, seen_list] = txt_sim[:, seen_list]
                txt_sim_unseen[:, unseen_list] = txt_sim[:, unseen_list]
                max_values_seen, _ = txt_sim_seen.max(dim=1)
                max_values_unseen, _ = txt_sim_unseen.max(dim=1)
                novelty_score = max_values_unseen - max_values_seen
                indices_1_txt_seen = txt_sim_seen.topk(1, dim=-1)[1]
                indices_1_txt_unseen = txt_sim_unseen.topk(1, dim=-1)[1]
                for i in range(len(label)):
                    if novelty_score[i] > -self.arg.novelty_threshold:
                        # unseen
                        if label[i] in indices_1_txt_unseen[i]:
                            # print(indices_1_txt_unseen[i])
                            unseen_correct += 1
                    else:
                        # seen
                        if label[i] in indices_1_txt_seen[i]:
                            seen_correct += 1
                    if label[i] in unseen_list:
                        unseen_num += 1
                    else:
                        seen_num += 1
        ...
        # self.io.print_log('\tAccuracy (zsl): {:.2f}%'.format(top1))
        top1_txt = float(corr_1_txt) / num * 100
        self.io.print_log('\tAccuracy (zsl txt_sim): {:.2f}%'.format(top1_txt))
        # self.io.print_log('\tLoss (zsl): {:.4f}'.format(np.mean(loss_value)))
        self.io.print_log('\tLoss (zsl txt_sim): {:.4f}'.format(np.mean(loss_txt_value)))
        # top1_s = float(corr_1_s) / num_s * 100
        # self.io.print_log('\tAccuracy (gzsl): {:.2f}%'.format(top1_s))
        # top1_txt_s = float(corr_1_txt_s) / num_s * 100
        # self.io.print_log('\tAccuracy (gzsl txt_sim): {:.2f}%'.format(top1_txt_s))
        # self.io.print_log('\tLoss (gzsl): {:.4f}'.format(np.mean(loss_value_s)))
        # self.io.print_log('\tLoss (gzsl txt_sim): {:.4f}'.format(np.mean(loss_txt_value_s)))
        ...
        seen_acc = seen_correct / seen_num * 100
        unseen_acc = unseen_correct / unseen_num * 100
        h = 2 * seen_acc * unseen_acc / (seen_acc + unseen_acc)
        self.io.print_log('\tAccuracy (seen): {:.2f}%'.format(seen_acc))
        self.io.print_log('\tAccuracy (unseen): {:.2f}%'.format(unseen_acc))
        self.io.print_log('\tH: {:.2f}%'.format(h))
        self.save_log[f'test_epoch{self.meta_info["epoch"] + 1}_support'] = {
            ...
            'accuracy_txt_gzsl': h,
            'accuracy_txt_zsl': top1_txt,
            # 'accuracy_txt_avg': top1_txt_avg
            # 'domain_accuracy': top1_domain
        }
        if self.arg.phase == 'train':
            if top1_txt > self.best_val_acc:
                self.best_val_acc = top1_txt
                self.io.save_model(self.model, 'best_model.pt')
                self.io.print_log('Best model saved.')
            if self.arg.gzsl_test_feeder_args:
                if h > self.best_val_acc_spt:
                    self.best_val_acc_spt = h
                    self.io.save_model(self.model, 'best_model_gzsl.pt')
                    self.io.print_log('Best gzsl_model saved.')
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                drop_last=True)
        if self.arg.zsl_test_feeder_args:
            self.data_loader['zsl_test'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.zsl_test_feeder_args),
                batch_size=self.arg.test_batch_size,
                shuffle=False,
                num_workers=self.arg.num_worker * torchlight.ngpu(
                    self.arg.device))
        if self.arg.gzsl_test_feeder_args:
            self.data_loader['gzsl_test'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.gzsl_test_feeder_args),
                batch_size=self.arg.test_batch_size,
                shuffle=False,
                num_workers=self.arg.num_worker * torchlight.ngpu(
                    self.arg.device))
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--w_cls', type=float, default=0.1, help='loss weights')
        parser.add_argument('--w_dann', type=float, default=0., help='loss weights')
        parser.add_argument('--w_clip', type=float, default=0.1, help='loss weights')
        parser.add_argument('--w_scl', type=float, default=0.1, help='loss weights')
        parser.add_argument('--w_froster', type=float, default=0.2, help='loss weights')
        parser.add_argument('--w_balance', type=float, default=1.0, help='loss weights')
        parser.add_argument('--w_balance_s', type=float, default=1.0, help='loss weights')
        parser.add_argument('--SGD_text', type=str2bool, default=True)
        parser.add_argument('--detach_support_text', type=str2bool, default=False)

        parser.add_argument('--gzsl_test_feeder_args', action=DictAction, default=dict())
        parser.add_argument('--zsl_test_feeder_args', action=DictAction, default=dict())
        parser.add_argument('--clip_loss_type', type=str, default='ce')
        parser.add_argument('--clip_loss_scale', type=float, default=1.0)
        parser.add_argument('--novelty_threshold', type=float, default=1.0)
