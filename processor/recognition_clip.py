...
class REC_Processor(Processor):
    ...
    def load_model(self):
        self.model = self.io.load_model(self.arg.model,
                                        **(self.arg.model_args))
        self.model.encoder.apply(weights_init)
    ...
    def adjust_lr(self):
        if self.arg.lr_decay_type == 'step':
            ...
            if self.arg.optimizer == 'Mix':
                ...
                for param_group in self.optimizer[1].param_groups:
                    param_group['lr'] = self.arg.base_lr * 10.
                for param_group in self.optimizer[2].param_groups:
                    param_group['lr'] = self.arg.base_lr
    ...
    def train(self):
        ...
        for data, label in tqdm(loader):
            ...
            # forward
            cls, cls_e, emb_norm, emb_e_norm, y_txt, y_txt_froster, y_txt_stu, _, _ = self.model(True, xs, xse, label)
            ...
            if self.model.extra_txt_data_view:
                # loss_clip = self.arg.w_clip * (self.loss_clip(emb_norm, y_txt[:len(emb_norm)]) +
                #                                self.loss_clip(emb_e_norm, y_txt[len(emb_norm):]))
                y_txt_1 = y_txt[:len(emb_norm)]
                y_txt_2 = y_txt[len(emb_norm):]
                loss_clip = self.arg.w_clip * (self.loss_clip(emb_norm, y_txt_1) +
                                               self.loss_clip(emb_e_norm, y_txt_2) +
                                               self.loss_clip(emb_norm, y_txt_2) +
                                               self.loss_clip(emb_e_norm, y_txt_1)) / 2.
            else:
                loss_clip = self.arg.w_clip * (self.loss_clip(emb_norm, y_txt) +
                                               self.loss_clip(emb_e_norm, y_txt))
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--clip_epoch', type=int, default=0)
        parser.add_argument('--SGD_text', type=str2bool, default=True)
        parser.add_argument('--SGD_skl', type=str2bool, default=False)
        parser.add_argument('--scl_epoch', type=int, default=0)
