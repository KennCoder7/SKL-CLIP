...
from net.utils.text_preprocess import text_preprocess
...
class Feeder(Dataset):
    def __init__(self, dataset, data_path, target_dataset=None, target_data_path=None,
                 ...
                 mini=-1, fusion_dataset=False, source_label_path=None, target_label_path=None,
                 fusion_label=True):
        """
        :param data_path:
        :param label_path:
        ...
        assert self.segment in ['none', 'all', 'source', 'target']
        ...
        if debug:
            ...
            if target_dataset is not None:
                ...
                self.target_label = self.target_label[:1000]

        if split == 'train' and backup:
            self.data_backup = self.data.copy()
            self.label_backup = self.label.copy()
            if target_dataset is not None:
                self.target_data_backup = self.target_data.copy()
                self.target_label_backup = self.target_label.copy()
        self.backup = backup
        self.mini_dataset()
    ...
    def mini_dataset(self):
        if self.mini == -1:
            ...
            if self.target_dataset is not None:
                self.target_data = self.target_data[self.target_label[:] < self.mini]
                self.target_label = self.target_label[self.target_label[:] < self.mini]

    def _fusion_dataset(self):
        if self.fusion_dataset:
            assert self.target_dataset is not None
            if self.fusion_label:
                source_label_name = np.load(self.source_label_path, allow_pickle=True)
                target_label_name = np.load(self.target_label_path, allow_pickle=True)

                # total_label_name = np.concatenate((source_label_name, target_label_name))
                # total_label_name = np.unique(total_label_name).tolist()
                # print(len(total_label_name))
                # label_mapping = {}
                # for i, name in enumerate(total_label_name):
                #     label_mapping[name] = i
                # label_name = np.concatenate((source_label_name[self.label], target_label_name[self.target_label]))
                # self.label = np.array([label_mapping[name] for name in label_name])

                #

                # 创建一个映射字典，将 source 标签映射到其索引
                label_mapping = {}

                # 计算源标签数量
                num_source_classes = np.unique(self.label).shape[0]
                num_target_classes = np.unique(self.target_label).shape[0]
                source_label_name = source_label_name[:num_source_classes]
                target_label_name = target_label_name[:num_target_classes]
                # source_label_name = source_label_name.tolist()
                # target_label_name = target_label_name.tolist()
                self.source_label_name = source_label_name
                self.target_label_name = target_label_name
                for i in range(len(self.source_label_name)):
                    self.source_label_name[i] = text_preprocess(self.source_label_name[i])
                for i in range(len(self.target_label_name)):
                    self.target_label_name[i] = text_preprocess(self.target_label_name[i])
                source_label_name_no_space = [name.replace(' ', '') for name in self.source_label_name]

                self.label_name = []
                self.label_name.extend(self.source_label_name)
                for i, name in enumerate(self.target_label_name):
                    if name not in self.source_label_name and name.replace(' ',
                                                                           '') not in self.source_label_name and name not in source_label_name_no_space:
                        self.label_name.append(name)
                    elif name in source_label_name_no_space:
                        self.target_label_name[i] = self.source_label_name[source_label_name_no_space.index(name)]
                label_mapping = {}
                for i, name in enumerate(self.label_name):
                    label_mapping[name] = i
                source_labels_name = [self.source_label_name[i] for i in self.label]
                target_labels_name = [self.target_label_name[i] for i in self.target_label]
                self.label = np.array([label_mapping[name] for name in source_labels_name + target_labels_name])
               # 预处理
               #  # print(source_label_name)
               #  # print(target_label_name)
               #  for i in range(len(source_label_name)):
               #      source_label_name[i] = text_preprocess(source_label_name[i])
               #  for i in range(len(target_label_name)):
               #      target_label_name[i] = text_preprocess(target_label_name[i])
               #  source_label_name_no_space = [name.replace(' ', '') for name in source_label_name]
               #  # print(source_label_name)
               #  # print(target_label_name)
               #  # exit()
               #
               #  # 创建一个新的标签数组
               #  for i, name in enumerate(target_label_name):
               #      if name in source_label_name or name.replace(' ', '') in source_label_name:
               #          label_mapping[i] = source_label_name.index(name)
               #      elif name in source_label_name_no_space:
               #          label_mapping[i] = source_label_name_no_space.index(name)
               #      else:
               #          label_mapping[i] = num_source_classes
               #          num_source_classes += 1
               #
               #  # 将列表转换为 NumPy 数组
               #  target_label_mapped = np.array([label_mapping[i] for i in self.target_label])
               #  self.label = np.concatenate((self.label, target_label_mapped), axis=0)


                # 合并数据和标签
                self.data = np.concatenate((self.data, self.target_data), axis=0)
                self.domain_label = np.concatenate((np.ones(self.data.shape[0] - self.target_data.shape[0]),
                                                    np.zeros(self.target_data.shape[0])), axis=0)
            else:
                num_source_classes = np.unique(self.label).shape[0]
                self.data = np.concatenate((self.data, self.target_data), axis=0)
                self.label = np.concatenate((self.label, self.target_label+num_source_classes), axis=0)
                self.domain_label = np.concatenate((np.ones(self.data.shape[0] - self.target_data.shape[0]),
                                                    np.zeros(self.target_data.shape[0])), axis=0)
    ...
    def reset_data(self):
        ...
        if self.target_dataset is not None:
            self.target_data = self.target_data_backup.copy()
            self.target_label = self.target_label_backup.copy()
    ...
    def _strong_aug(self, data_numpy, dataset):
        ...
        if '2' in self.strong_aug_method:
            data_numpy = f_random_rot(data_numpy, r=0.1)
    ...
    def __getitem__(self, index):
        ...
        # data_numpy = self._segment(data_numpy, 'source')
        ...
        if self.get_pairs:  # v2
            ...
            # data_numpy_2 = self._segment(data_numpy_2, 'source')
            data_numpy = (data_numpy, data_numpy_2)

        if self.fusion_dataset:
            domain_label = self.domain_label[index]
            return data_numpy, label, domain_label
        ...
        if self.target_data_path is not None:
            ...
            target_data_numpy = self._augment(target_data_numpy)
            # target_data_numpy = self._segment(target_data_numpy, 'target')
            if self.get_pairs:
                target_data_numpy_2 = self.target_data[target_index]
                target_data_numpy_2 = np.array(target_data_numpy_2)
                target_data_numpy_2 = self._augment(target_data_numpy_2)
                target_data_numpy_2 = self._strong_aug(target_data_numpy_2, self.target_dataset)
                # target_data_numpy_2 = self._segment(target_data_numpy_2, 'target')
