...
def index_alignment(similarity_matrix):
    ...
    return (A2B, 'source') if len(A2B) < len(B2A) else (B2A, 'target')
...
def plot_similarity_matrix(softmax_similarity, nameA, nameB,
                           ...
                           figsize=(12, 12),
                           save_path='Similarity Matrix.pdf'):
    # 设置绘图的大小
    plt.figure(figsize=figsize)

    # 使用seaborn的heatmap绘制相似度矩阵，annot=True显示每个格子的数值
    # sns.heatmap(softmax_similarity, xticklabels=nameB, yticklabels=nameA, cmap='viridis',
    #             annot=True, fmt='.2f')

    # toys
    # cmap type: 'viridis', 'Blues', 'coolwarm', 'RdYlBu', 'cividis'
    sns.heatmap(softmax_similarity, cmap='Blues',
...
def ntu12ANDposetics12():
    ...
    plot_similarity_matrix(similarity_matrix, ntu_label_chosen, posetics_chosen,
                           ...
                           xlabel='Posetics12',
                           ylabel='NTU12')
    index_st, sORt = index_alignment(similarity_matrix)
    print(index_st, sORt)
...
def ntu60ANDpku51():
    ...
    #          label_embedding=output_ntu.cpu().detach().numpy(), label_name=np.array(ntu_label_chosen))
    ntu_label_emb = np.load('/data/wangkun/project/datasets/NTU_dataset/ntu/true_label.npz',
                            allow_pickle=True)['label_embedding']
    ntu_label_emb = torch.tensor(ntu_label_emb).to('cuda')
    print(ntu_label_emb.shape)
    ntu_cls_center = np.load('/data/wangkun/project/work_dir/LowQSAR/DA-CL-dual-txt-share/ntu60ANDpku51/cs-1025-v6-baseline1-vis-source/centers.npy')
    ntu_cls_center = torch.tensor(ntu_cls_center).to('cuda').unsqueeze(1)
    print(ntu_cls_center.shape)
    ...
    # np.savez('/data/wangkun/project/datasets/PKUMMD/pku_part1/true_label.npz',
    #          label_embedding=output_p.cpu().detach().numpy(), label_name=np.array(pku_label))
    pku_label_emb = np.load('/data/wangkun/project/datasets/PKUMMD/pku_part1/true_label.npz',
                            allow_pickle=True)['label_embedding']
    pku_label_emb = torch.tensor(pku_label_emb).to('cuda')
    print(pku_label_emb.shape)
    pku_cls_center = np.load('/data/wangkun/project/work_dir/LowQSAR/DA-CL-dual-txt-share/ntu60ANDpku51/cs-1025-v6-baseline1-vis-target/centers.npy')
    pku_cls_center = torch.tensor(pku_cls_center).to('cuda').unsqueeze(1)
    print(pku_cls_center.shape)

    np.random.seed(0)
    rand_idx = np.random.choice(len(pku_label), 5, replace=False)
    ntu_label_emb = ntu_label_emb[rand_idx]
    ntu_cls_center = ntu_cls_center[rand_idx]
    pku_label_emb = pku_label_emb[rand_idx]
    pku_cls_center = pku_cls_center[rand_idx]
    ntu_label_chosen = ntu_label_chosen[rand_idx]
    pku_label = pku_label[rand_idx]

    ntu = torch.cat([ntu_label_emb, ntu_cls_center], dim=1)
    pku = torch.cat([pku_label_emb, pku_cls_center], dim=1)
    print(ntu.shape, pku.shape)

    view_weight = [0., 1.]
    similarity_matrix = entity_alignment(ntu, pku,
                                         view_weight=view_weight).to('cpu').detach().numpy()
    # print(similarity_matrix)
    plot_similarity_matrix(similarity_matrix, ntu_label_chosen, pku_label,
                           title=f'Similarity Matrix between NTU60 and PKU51 ($a$={view_weight[0]}, $b$={view_weight[1]})',
                           xlabel='PKU51',
                           ylabel='NTU60',
                           figsize=(5, 5),
                           save_path='/data/wangkun/project/work_dir/LowQSAR/vis/sm_toy_feat.pdf')
    index_alignment(similarity_matrix)
...
if __name__ == '__main__':
    ...
    # with open('/data/wangkun/project/work_dir/LowQSAR/DA-CL-dual-txt-share/ntu60ANDpku51/cs-1025-v7/log.pkl', 'rb') as f:
    #     log = pickle.load(f)
    # /data/wangkun/project/work_dir/LowQSAR/DA-CL-dual-txt-share/ntu60ANDpku51/cs-1025-v7-scl/log.pkl
    # with open('/data/wangkun/project/work_dir/LowQSAR/DA-CL-dual-txt-share/ntu60ANDpku51/cs-1025-v7-scl/log.pkl', 'rb') as f:
    #     log = pickle.load(f)
    # print(log.keys())
    # similarity_matrix = log['sm_epoch40']
    # print(similarity_matrix.shape)
    # ntu_label_chosen = np.load('/data/wangkun/project/datasets/NTU_dataset/label_list.npy', allow_pickle=True)
    # ntu_label_chosen = ntu_label_chosen[:60]
    # pku_label = np.load('/data/wangkun/project/datasets/PKUMMD/pku_part1/label_list.npy', allow_pickle=True)
    # plot_similarity_matrix(similarity_matrix, ntu_label_chosen, pku_label,
    #                        title=f'Similarity Matrix between NTU60 and PKU51',
    #                        xlabel='PKU51',
    #                        ylabel='NTU60',
    #                        figsize=(24, 24),
    #                        save_path='/data/wangkun/project/work_dir/LowQSAR/vis/scl-sm_epoch40.pdf')